...
class BeeColony:
    def __init__(self, args):
        ...
        # compute straight‐line distance & tight threshold
        ...
        # sample dest orbit + angular weights
        ...
        # patches: include true dest idx + extras
        idxs = {di}
    ...
    def run(self):
        ...
        for _ in range(steps):
            ...
            for b in self.employed:
                ...
                dest_xy = self.sim.grav.traj[self.bonus_idxs[0],0,:2]
        ...
        # after sim, return nothing—the caller will pick the winner’s path
        return
...
def main():
    ...
    for ep in range(1, args.epochs+1):
        ...
        colony.run()
        # pick winner bee by final fitness
    ...
    # pull out the winning colony+bee
    best_col, best_bee = overall_best_bee

    # truncate path at arrival
    path = np.vstack(best_bee.path)
    dest_xy = best_col.sim.grav.traj[
        best_col.bonus_idxs[0], 0, :2
    ]
    d2  = np.linalg.norm(path - dest_xy, axis=1)
    hit = np.where(d2<0.02)[0]
    if hit.size:
        path = path[:hit[0]+1]

    # write fitness time‐series
    save_fitness(best_col, os.path.join(out_sub,"fitness.csv"))

    # animation of the winning colony
